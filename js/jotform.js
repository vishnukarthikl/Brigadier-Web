...
JotForm = {
    ...
    handleScreenshot: function(){
        setTimeout(function(){
            $$('.form-screen-button').each(function(button){
                JotForm.getContainer(button).hide();
                // If window parent has feedback then show screenshot
                if(window.parent && window.parent.JotformFeedbackManager){
                    JotForm.getContainer(button).show();                
                    button.observe('click', function(){
                        try{
                            JotForm.takeScreenShot(button.id.replace('button_', ''));
                        }catch(e){
                            console.error(e);
                        }
                    });
    ...
    takeScreenShot: function(id){
        ...
        this.wishboxServer = 'http://screenshots.jotform.com/wishbox-bot.php';
        ...
        var form = new Element('form', {action:this.wishboxServer, target:'screen_frame', id:'screen_form', method:'post'}).hide();
        ...
        p.$jot('<div id="js_loading" '+
          'style="position:fixed; z-index:10000000; text-align:center; background:#333; border-radius:5px; top: 20px; right: 20px; padding:10px; box-shadow:0 0 5 rgba(0,0,0,0.5);">'+
          '<img src="'+this.url+'images/loader-black.gif" /><div style="font-family:verdana; font-size:12px;color:#fff;">Please Wait</div></div>').appendTo('body');
        ...
        form.insert(html).insert(height).insert(scrollTop).insert(action).insert(uid).insert(url).insert(fid);
    ...
    checkScreenShot: function(){
        ...
        p.$jot.getJSON(this.wishboxServer+'?callback=?', {
            ...
            function(data) {
                if(data.success === true){
                    $this.injectEditor(data.data); // If screenshot is created inject the editor on the page
                    // Remove the form and iframe since we don't need them anymore
                    $('screen_form').remove();
                    $('screen_frame_id').remove();
                }else{
                    if(data.status == 'working' && --count){
                        ...
                        alert('We are having technical problems right now. Please try again later.');
    ...
    injectEditor: function(data){
        ...
        p.$jot.getJSON(this.server+'?callback=?', {
            ...
            function(res) {
                ...
                var editorFrame = p.$jot('<iframe allowtransparency="true" src="" frameborder="0" style="width:100%; height:100%; border:none; position:fixed; top:0; left:0;z-index:100000000;" scrolling="no"></iframe>').appendTo('body');
                ...
                p.jotformScreenshotURL = data;//JotForm.url+'shots/'+$this.scuniq+'.jpg';
                ...
                p.JotformFinishEditing = function(image){
                    ...
                    $this.injected = false;
                    $('screen_'+$this.scID).update('<img width="100%" align="center" src="'+image+'" />');
    ...
    */
    ...
    FBInit: function(){
        ...
        FB.getLoginStatus(function(response) {
            if (response.session) { // user is already logged in
